...
export default function SidebarNavigation({
  ...
  return (
    <div className="md:block block p-[20px] md:w-[550px] bg-white border-r border-gray-200 left-0 top-0 z-10">
      <div
        className="p-6 my-8 mx-3 bg-[#ddeffd] rounded-md"
      ...
      >
          ...
          {openAccordion === "Collect Payments" && (
            <div className="py-2">
              <nav className="space-y-1 overflow-y-auto overflow-x-hidden">
                {["Payment Gateway", "Payin Services", "Payout Services"].map(
                  (name, i) => (
                    <button
                      key={i}
                      type="button"
                      onClick={(e) => handleSelect(name, "Collect Payments", e)}
                      className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors w-full text-left ${
                        isActive(name)
                          ? "bg-[#f8f1ee] text-[#ab6545] mr-2"
        ...
        <div className="border-0 border-b-2 rounded-none mt-4">
          ...
          {openAccordion === "APIs" && (
            <div className="py-2">
              <nav className="space-y-1 h-[150px] overflow-x-hidden  overflow-y-auto">
                {["Recharge", "DMT", "Flight Booking", "Hotel Booking"].map(
                  (name, i) => (
                    <button
                      key={i}
                      type="button"
              ...
              <nav className="space-y-1 h-[150px] overflow-x-hidden  overflow-y-auto">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={(e) =>
                      handleSelect(name, "Website Development Services", e)
                    }
                    className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors w-full text-left ${
                      isActive(name)
                        ? "bg-[#f8f1ee] text-[#ab6545] mr-2"
              ...
              <nav className="space-y-1 h-[150px] overflow-x-hidden  overflow-y-auto">
                {["Android App", "IOS App"].map((name, i) => (
                  <button
                    ...
                    type="button"
                    onClick={(e) =>
                      handleSelect(name, "App Development Services", e)
                    }
                    className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors w-full text-left ${
                      isActive(name)
                        ? "bg-[#f8f1ee] text-[#ab6545] mr-2"
